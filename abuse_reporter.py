...
def save_ip_to_db(ip, domain, first_seen, last_seen, cf_banned=0, abuse_meta=None, last_comment=None, cf_rule_id=None, cf_ban_mode=None, cf_ban_expires_at=None):
    ...
    cursor.execute(
        """INSERT OR REPLACE INTO reported_ips
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (
            ip, domain, to_iso(now_utc()), cf_banned, first_seen, last_seen,
            abuse_meta.get("abuseConfidenceScore"), abuse_meta.get("totalReports"),
            abuse_meta.get("lastReportedAt"), last_comment,
...
def upsert_ip_cache(ip, **fields):
    ...
    cursor.execute(
        """INSERT OR REPLACE INTO ip_cache
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (
            ip, existing.get("reverse_hostname"), existing.get("reverse_checked_at"),
            existing.get("abuse_json"), existing.get("abuse_checked_at"), existing.get("asn"),
            existing.get("usage_type"), existing.get("abuse_confidence_score"), existing.get("total_reports"),
            existing.get("last_reported_at"), existing.get("country_code"), existing.get("is_whitelisted"),
            existing.get("is_public"), existing.get("is_tor"),
        )
    )
    conn.commit()
    conn.close()
...
def is_private_or_reserved(ip):
    try:
        ...
        return any([
            ...
            ip_obj.is_multicast, ip_obj.is_reserved, ip_obj.is_unspecified,
        ])
    except ValueError:
        return True
...
def abuse_request(method, endpoint, *, params=None, data=None):
    ...
    if response.status_code == 429:
        ...
        print(f"[RATE][{endpoint}] limit={response.headers.get('X-RateLimit-Limit')} remaining={response.headers.get('X-RateLimit-Remaining')} reset={response.headers.get('X-RateLimit-Reset')} retry_after={retry_after}")
...
def abuseipdb_check_ip(ip, max_age_days=ABUSE_CHECK_MAX_AGE_DAYS, verbose=False, use_cache=True):
    ...
    upsert_ip_cache(ip, abuse_json=json.dumps(data, ensure_ascii=False), abuse_checked_at=to_iso(now_utc()), asn=(str(data.get("asn")).upper() if data.get("asn") else None), usage_type=data.get("usageType"), abuse_confidence_score=data.get("abuseConfidenceScore"), total_reports=data.get("totalReports"), last_reported_at=data.get("lastReportedAt"), country_code=data.get("countryCode"), is_whitelisted=1 if data.get("isWhitelisted") is True else 0 if data.get("isWhitelisted") is False else None, is_public=1 if data.get("isPublic") is True else 0 if data.get("isPublic") is False else None, is_tor=1 if data.get("isTor") is True else 0 if data.get("isTor") is False else None)
    with run_cache_lock:
        run_abuse_cache[cache_key] = data
    return data
...
def build_report_comment(domain, data, report_tag):
    ...
    comment = (f"{report_tag}Automated report: malicious web probing against {domain}. " f"Methods: {methods}. HTTP statuses: {status_codes}. " f"Attempted URI(s): {sampled_urls}. Matched patterns: {triggers_str}. " f"First seen: {data['first_seen']}. Last seen: {data['last_seen']}. " f"Request count: {data['count']}.")
...
def ban_in_cloudflare(ip, cf_zone_id):
    ...
    url = f"https://api.cloudflare.com/client/v4/zones/{cf_zone_id}/firewall/access_rules/rules"
    body = {"mode": "block", "configuration": {"target": "ip6" if ":" in ip else "ip", "value": ip}, "notes": "Automated ban by abuse_reporter.py for malicious web probing"}
    ...
    with open(log_file, "r", encoding="utf-8", errors="ignore") as f:
        ...
        for line in f:
            ...
            if ip not in malicious_activity:
                malicious_activity[ip] = {"count": 0, "urls": set(), "triggers": set(), "methods": set(), "statuses": set(), "first_seen": date_iso, "last_seen": date_iso}
